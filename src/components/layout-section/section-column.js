  ...
  return {
    renderNode: {
      ...
      [BLOCKS.EMBEDDED_ASSET]: (node) => {
        ...
        switch (mimeGroup) {
          case 'image':
            return <img className={`${styles.image}`} style={options}
...
const SectionColumn = (props) => {
  ...
  return (
    <div className={getColumnClasses(column.columnType)}>
      {documentToReactComponents(column.childContentfulSectionColumnColumnContentRichTextNode.json, richTextOptions(props.options))}
    </div>
  )
}
